...
# 2. try to build, sign and send a transaction with ttl in the past (=tip - 1)
...
amount_transferred=$(( highest_utxo_amount_balance - fee - 100 ))
...
if [ $?	== 0 ]; then
	error_msg "Transaction with TTL in the past; Expected error when submitting transaction"
	exit 1
else
	success_msg "Transaction with TTL in the past could not be submitted"
fi