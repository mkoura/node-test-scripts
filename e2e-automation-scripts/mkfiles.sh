...
sed -i ${ROOT}/genesis.spec.json \
    ...
    -e 's/"poolDeposit": 0/"poolDeposit": 0/'
...
tmux send-keys -t "$SESSION_NAME" "cardano-node run --config example/configuration.yaml --topology example/node-pool1/topology.json --database-path example/node-pool1/db --socket-path example/node-pool1/node.sock --shelley-kes-key example/node-pool1/kes.skey --shelley-vrf-key example/node-pool1/vrf.skey --shelley-operational-certificate example/node-pool1/node.cert --port 3003" C-m

tmux select-pane -t 1
tmux send-keys -t "$SESSION_NAME" "cardano-node run --config example/configuration.yaml --topology example/node-bft1/topology.json --database-path example/node-bft1/db --socket-path example/node-bft1/node.sock --shelley-kes-key example/node-bft1/kes.skey --shelley-vrf-key example/node-bft1/vrf.skey --shelley-operational-certificate example/node-bft1/node.cert --port 3001" C-m

tmux select-pane -t 2
tmux send-keys -t "$SESSION_NAME" "cardano-node run --config example/configuration.yaml --topology example/node-bft2/topology.json --database-path example/node-bft2/db --socket-path example/node-bft2/node.sock --shelley-kes-key example/node-bft2/kes.skey --shelley-vrf-key example/node-bft2/vrf.skey --shelley-operational-certificate example/node-bft2/node.cert --port 3002" C-m

tmux select-pane -t 3
tmux send-keys -t "$SESSION_NAME" "sleep 40; CARDANO_NODE_SOCKET_PATH=example/node-bft1/node.sock cardano-cli shelley transaction submit --tx-file example/tx1.tx --testnet-magic 42" C-m
tmux send-keys -t "$SESSION_NAME" "sleep 5; ./e2e-automation-scripts/create-payment-and-stake-addresses-with-registartion.sh" C-m
